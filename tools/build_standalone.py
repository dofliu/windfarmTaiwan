...
def site_version():
    """core.js 的 WW.VERSION；兩份 CHANGELOG 都要有這個版本的段落（## vX.Y.Z），否則中止：
    改版號時一定要一起寫更新紀錄。
    WW.VERSION in core.js; both changelogs must have a "## vX.Y.Z" entry for it, or the build stops."""
    m = re.search(r"^WW\.VERSION = '(\d+\.\d+\.\d+)';", read("assets/js/core.js"), re.M)
    if not m:
        sys.exit("WW.VERSION not found in assets/js/core.js")
    v = m.group(1)
    for fn in ("CHANGELOG.md", "CHANGELOG.en.md"):
        if not re.search(r"^## v" + re.escape(v) + r"\b", read(fn), re.M):
            sys.exit(f"{fn} has no '## v{v}' entry; add it to both CHANGELOG.md and CHANGELOG.en.md when changing WW.VERSION")
    return v
...
def main():
    ...
    header = (f"<!-- 風電風情 單檔版 v{ver} / Wind Watch single-file edition v{ver} — generated by tools/build_standalone.py "
    ...
    print(f"wrote {OUT.relative_to(ROOT)}  {size / 1e6:.2f} MB  (v{ver}, built {built}, {sha})")
