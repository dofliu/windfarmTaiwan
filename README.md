...
- `wind_archive_daily.json` — 長期存檔的每日摘要（avg/max/min MW，約 6KB），前端「圖表 → 長期趨勢」讀這個
...
- `grid_status.json` — 電力供需即時報表（尖峰負載/供電能力/備轉容量率），與風力資料同一支 scraper、同排程產生；
  **無政府開放資料授權端點可用**，僅原始網頁資料一種來源，欄位無法辨識時不寫檔（前端該區塊會自動隱藏，不顯示臆測值）
